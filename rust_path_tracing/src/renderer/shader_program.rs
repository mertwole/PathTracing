...
impl ShaderProgram {
    ...
    pub fn compile(&self) -> GLuint {
        unsafe {
            ...
            if *log_len != 0 {
                panic!(
                    "log : {}",
                    String::from_utf8((*(&log as *const [i8; 512] as *const [u8; 512])).to_vec())
                        .unwrap()
                );
            }
        }
    ...
    pub fn add_frag(self, source: String) -> ShaderProgram {
        unsafe {
            let _ = &self.add_shader(source, gl::FRAGMENT_SHADER);
        }
        self
    }
    ...
    pub fn add_vert(self, source: String) -> ShaderProgram {
        unsafe {
            let _ = &self.add_shader(source, gl::VERTEX_SHADER);
        }
        self
    }
